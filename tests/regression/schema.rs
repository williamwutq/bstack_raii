...
fn deep_clone_aliases_exactly_the_shared_shapes() {
    ...
    let rtti_clone: Wide = {
        ...
        let off = unsafe { reg.clone_value(&alloc, ord, root.handle().range().start()) }.unwrap();
        unsafe {
...
fn rtti_teardown_frees_what_the_static_walk_frees() {
    ...
    for (name, off, len) in &rec {
        let r = unsafe { bstack_raii::dealloc_range(&alloc, bstack::BStackRange::new(*off, *len)) };
        match r {
            Ok(()) => {
...
use bstack_raii::{BStackBlock, BStackDrop, BStackOwned, TryCloneIn, bstack_block, bstack_enum};
...
fn run(name: &str, build: impl Fn(&A, &mut Vec<u64>) -> BStackOwned<E>, expect_owned: bool) {
    ...
    println!("  {verdict} {name:<10} children {offs:?}  still allocated after drop: {leaked:?}");
    if expect_owned {
